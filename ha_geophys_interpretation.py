...
/***************************************************************************
 HAGeophysInterpretation
                                 A QGIS plugin
 This plugin can help digitise pixels data to polygon.
...
from qgis.core import QgsSingleSymbolRenderer, QgsFillSymbol, QgsRasterLayer, QgsMapLayerProxyModel, QgsCoordinateReferenceSystem, QgsProject
from .exclusive_checkbox import toggle_checkboxes
from .ha_process_raster import process_pixel
import processing
...
class HAGeophysInterpretation:
    ...
    def update_parameters(self):
        if self.dlg.tifCheckBox.isChecked():
            ...
            self.dlg.disturbanceParameter.setVisible(is_checked)
    ...
    def run(self):
        ...
        self.dlg.disturbanceParameter.setVisible(self.dlg.pngCheckBox.isChecked() or self.dlg.tifCheckBox.isChecked())
        ...
        if result:
            ...
            disturbanceBiggerValue = float(self.dlg.biggerThanValue.text()) 
            disturbanceSmallerValue = float(self.dlg.smallerThanValue.text())
            base_name = os.path.splitext(raster_layer.name())[0]
            inputCRS = f"EPSG:{self.dlg.newEPSGinput.text()}"
            TARGET_CRS = QgsCoordinateReferenceSystem(inputCRS) if self.dlg.newEPSGinput.text() else QgsCoordinateReferenceSystem("EPSG:27700")

            


            polygon_area_exp = f'"area" IS NOT NULL AND "area" > {polygonMinArea}'
            disturbance_value_exp = f'"VALUE" > {disturbanceBiggerValue} OR "VALUE" < {disturbanceSmallerValue}'
            disturbance_area_exp = '"area" IS NOT NULL'
            if tifChecked:
            ...
            # Check if we need to change the CRS or use default one
            epsg_text = self.dlg.newEPSGinput.text().strip()
            if epsg_text:
                crsValue = f"EPSG:{epsg_text}"
            else:
                crsValue = "EPSG:27700"

            # Add the polygon layer
            filtered_polygon = process_pixel(raster_layer,polygon_value_exp,polygon_area_exp, crsValue )
            filtered_polygon.setCrs(TARGET_CRS)
            filtered_polygon.setName(f"Int_Polygon_{base_name}_{polygonThreshold}_{polygonMinArea}")
        

            filtered_polygon_for_disturbance, disturbance_polygon = process_pixel(
                raster_layer, disturbance_value_exp, disturbance_area_exp, crsValue, disturbance='Yes'
            )

            disturbance_polygon.setCrs(TARGET_CRS)
            disturbance_polygon.setName(f"Edge_Disturbance_{base_name}_>{disturbanceBiggerValue}_OR_<{disturbanceSmallerValue}")
